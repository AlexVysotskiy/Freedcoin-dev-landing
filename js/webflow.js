  ...
  Webflow.define('links', module.exports = function($, _) {
    ...
    function select(link) {
      ...
      if (href.indexOf('#') === 0 && validHash.test(href)) {
        var $section = $(href);
        var $additionalSec = $('[data-section="'+href+'"]');
        $section.length && anchors.push({ link: $link, sec: $section, addSec: $additionalSec, active: false });
        return;
      }
...
Webflow.require('ix').init([
  ...
  {"slug":"fade-in-page-loads-2","name":"Fade in (page loads) 2","value":{"style":{"opacity":0},"triggers":[{"type":"load","stepsA":[{"opacity":1,"transition":"transform 1000ms ease 0ms, opacity 1000ms ease 0ms","x":"0px","y":"0px","z":"0px"}],"stepsB":[]}]}},
  {"slug":"fade-in-delay","name":"Fade in  delay","value":{"style":{"opacity":0},"triggers":[{"type":"scroll","offsetBot":"20%","stepsA":[{"wait":"150ms"},{"opacity":1,"transition":"transform 1000ms ease 0ms, opacity 1000ms ease 0ms","x":"0px","y":"0px","z":"0px"}],"stepsB":[]}]}},
  {"slug":"fade-in-delay-2","name":"Fade in  delay 2","value":{"style":{"opacity":0},"triggers":[{"type":"scroll","offsetBot":"20%","stepsA":[{"wait":"300ms","transition":"transform 200 ease 0"},{"wait":"100ms","opacity":1,"transition":"transform 1000ms ease 0ms, opacity 1000ms ease 0ms","x":"0px","y":"0px","z":"0px"}],"stepsB":[]}]}},
  {"slug":"fade-in-delay-3","name":"Fade in  delay 3","value":{"style":{"opacity":0},"triggers":[{"type":"scroll","offsetBot":"20%","stepsA":[{"wait":"450ms"},{"wait":"200ms","opacity":1,"transition":"transform 1000ms ease 0ms, opacity 1000ms ease 0ms","x":"0px","y":"0px","z":"0px"}],"stepsB":[]}]}},
  ...
  {"slug":"fade-in-bottom-scroll-in","name":"Fade in bottom (scroll in)","value":{"style":{"opacity":0,"x":"0px","y":"50px","z":"0px"},"triggers":[{"type":"scroll","offsetBot":"20%","stepsA":[{"opacity":1,"transition":"transform 1000ms ease 0ms, opacity 1000ms ease 0ms","x":"0px","y":"0px","z":"0px"}],"stepsB":[]}]}},
  {"slug":"fade-in-bottom-scroll-in-2","name":"Fade in bottom (scroll in) 2","value":{"style":{"opacity":0,"x":"0px","y":"50px","z":"0px"},"triggers":[{"type":"scroll","offsetBot":"20%","stepsA":[{"wait":"300ms"},{"opacity":1,"transition":"transform 1000ms ease 0ms, opacity 1000ms ease 0ms","x":"0px","y":"0px","z":"0px"}],"stepsB":[]}]}},
...
$(document).ready(function(){
    ...
    //timer
    if($('.countdown').length) {
      // Р—Р°РґР°РµРј С‚РѕС‡РєСѓ РѕС‚СЃС‡РµС‚Р°.
      var ts = new Date(2017, 9, 26, 17, 0, 0, 0);
      /* РџСЂРёРІРѕРґРёРј РІСЂРµРјСЏ Рє РњРЎРљ */
      var currentTime = (180 + ts.getTimezoneOffset())*60*1000;
      ts = new Date(ts.getTime()-currentTime);
      // Р’С‚РѕСЂР°СЏ РґР°С‚Р°
      var sts = new Date(2017, 9, 31, 17, 0, 0, 0);
      sts = new Date(sts.getTime()-currentTime);
      var today = new Date();
      if(today > ts) {
          if(lang == 'eng') {
              $('.countdown-header').html('Pre-ICO ends in')
          } else {
              $('.countdown-header').html('Pre-ICO Р·Р°РєРѕРЅС‡РёС‚СЃСЏ С‡РµСЂРµР·')
          }
      }
	...
	if(lang == 'eng') {
        ...
        var videoURL = 'https://www.youtube.com/embed/e9NSeeMYWUo';
    }

	var $videopopup = $('<div />');
	$videopopup.addClass('videopopup').addClass('videopopup-main').appendTo('body');
	$videopopup.html('<iframe autoplay=1" src="'+videoURL+'" frameborder="0" allowfullscreen></iframe>');

	var $videoclose = $('<a class="videopopup videoclose videoclose-button">Close</a>');
	$videoclose.appendTo('body');

	var $videopopupBg = $('<div />');
	$videopopupBg.addClass('videopopup').addClass('videopopup-bg').addClass('videoclose').appendTo('body');	

	$('div.video div').on('click', function(){
	...
	$('a.contact-button').on('click', function(){
        ga('send', 'event', lang, 'buy', 'pop-up');
		$('.buypopup').addClass('active');
		resizeWin();
	});
    ...
    $('.faq-category-question').on('click', function(event){
      ...
      if($target.hasClass('active')) {
        ...
        $('.faq-category-answer').slideUp('500');
      } else {
        $('.faq-category-question.active').removeClass('active');
        $target.addClass('active');
        $('.faq-category-answer').slideUp('500');
        $target.parent('.faq-category-question-wrap').next('.faq-category-answer').slideDown('500');
      }
    });

	$('.buyclose').on('click', function(){
		$('.buypopup').removeClass('active');
		$('.buypopup').find('form')[0].reset();
		$('.buypopup').find('form').show();
		$('.buypopup').find('.form-message-buy').hide();
	});

	/*$('.bck-table').on('scroll', function(){
		if($(this).hasClass('scrollfirst'))
		{
			$(this).removeClass('scrollfirst');
			$(this).addClass('scrollsecond');
		}
	})*/

	// Р·Р°РєСЂС‹С‚СЊ РІРёРґР¶РµС‚ РїРѕРґРїРёСЃРєРё РІ РјРѕР±РёР»СЊРЅРѕР№ РІРµСЂСЃРёРё
    $('.subscribe-sector.top .close').click(function (event) {
      console.log($(event.currentTarget).parent('.top'));
        $(event.currentTarget).parent('.top').css('height', 0);
    });
...
function resizeWin()
{
	$('.videopopup-main:visible').each(function(){
		var h = $(this).height() / 2;
		$(this).css('margin-top', '-'+Math.floor(h)+'px');
	});
	...
	if(scrollBlock.clientWidth != scrollBlock.scrollWidth)
	{
		if(!$('.bck-table').hasClass('scrollsecond'))
			$('.bck-table').addClass('scrollfirst');
	}
	else
	{
		$('.bck-table').removeClass('scrollfirst').removeClass('scrollsecond');	
	}*/

	var whatToUse = Math.min($('.hero.section').height(), window.innerHeight);
	$('.hero-social').css('top', (whatToUse / 2 - 80)+'px');
}
